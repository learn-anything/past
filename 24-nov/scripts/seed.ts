...
import {
  ...
  ListOfConnections,
  ListOfForceGraphs,
} from "@/web/lib/schema/master/force-graph"
import {
  PublicGlobalGroup,
  PublicGlobalGroupRoot,
} from "@/web/lib/schema/master/public-group"
import {
  LatestGlobalGuide,
  Link,
  ListOfLinks,
...
async function loadGlobalGroup() {
  ...
  const globalGroupId = getEnvOrThrow(
    "JAZZ_PUBLIC_GLOBAL_GROUP"
 ...
 */
async function saveForceGraph(
  connectionLists: Connection[],
  forceGraphs: ForceGraphJson[]
) {
  const globalGroup = await loadGlobalGroup()
...
async function forceGraphSeed() {
  ...
  console.log(
    `Collected ${
      connectionManager.getAllConnections().length
    } unique connections.`
  )
  console.log(
    `Found ${connectionManager.getDuplicateCount()} duplicate connections.`
  )

  const insertedConnections = await insertConnectionsInBatch(
